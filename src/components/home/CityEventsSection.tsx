...
export const CityEventsSection = ({ city }: { city: any }) => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        setLoading(true);
        const endpoint = `${BASE_URL}/events/city/${city.slug}`;
  ...
  return (
    <div className="relative" style={{ minHeight: '380px' }}>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-2xl font-bold text-gray-900 capitalize">{city.name}</h3>
      </div>
