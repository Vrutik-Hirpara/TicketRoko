...
import { UPLOADS_URL } from '../../../src/utils/constants';
import { AppDispatch, RootState } from '../../../src/store';
import { fetchEventById } from '../../../src/controllers/eventController';
...
export default function EventDetailPage() {
  ...
  const eventId = Number(params?.slug);
  ...
  // Dynamically resolve upload paths — reads UPLOADS_URL from env via constants.ts
  const getFullImageUrl = (url?: string | null) => {
    if (!url) return 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1200&auto=format&fit=crop&q=60';
    if (url.startsWith('http://') || url.startsWith('https://')) return url;
    return `${UPLOADS_URL}/uploads/events/${url}`;
  };
